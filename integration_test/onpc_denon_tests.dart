...
class OnpcDenonTests {
  static final String DENON_AVR = "Denon AVR";
  static final String FRIENDLY_NAME = "My Denon AVR";
  ...
  static Future<void> playFromUsb(final OnpcTestUtils tu) async {
    ...
    await tu.findAndTap(() => find.widgetWithText(ListTile, DENON_AVR), ensureAfter: () => find.byTooltip("Search"));

    // Search Accept
    final String artist1 = "Accept";
    await tu.openSearchDialog(0, artist1);

    // Add first albums to the queue
    final String album1 = "Metal Heart";
    await tu.findAndTap(() => find.widgetWithText(ListTile, artist1),
    ...
    await tu.openSearchDialog(1, album2);
    expect(find.text(artist1), findsNothing);
    expect(find.text(album1), findsNothing);
    await tu.contextMenu(album2, "Add");

    // Inspect queue
    await tu.findAndTap(() => find.byTooltip("Top Menu"), ensureAfter: () => find.text("Play Queue"));
    await tu.findAndTap(() => find.text("Play Queue"),
    ...
    // Stop
    await tu.findAndTap(() => find.byTooltip("Stop"));
    ...
    // Audio info dialog
    await tu.findAndTap(() => find.text("Stereo".toUpperCase()), delay: OnpcTestUtils.NORMAL_DELAY);
    await tu.ensureAvInfo("MP3 44.1 kHz", "Stereo");
  }

  static Future<void> audioControlMain(OnpcTestUtils tu) async {
    await _audioControl(tu, true);
  }

  static Future<void> _audioControl(OnpcTestUtils tu, bool mainZone) async {
    ...
    // All zones
    await tu.findAndTap(() => find.byTooltip(Strings.audio_control_all_zones),
    ...
    // Channel level
    await tu.findAndTap(() => find.byTooltip(Strings.audio_control_channel_level),
  ...
  static Future<void> testHeosFavorites(OnpcTestUtils tu) async {
  ...
  static Future<void> playFromDeezer(OnpcTestUtils tu) async {
    ...
    await tu.findAndTap(() => find.text(toPlay));
    await tu.waitMediaItemPlaying(toPlay);
  ...
  static Future<void> hideEmptyItems(OnpcTestUtils tu) async {
    await tu.openTab("MEDIA", ensureAfter: () => find.text("NET"));

    // Search artist
    final String artist = "Muse";
    await tu.navigateToMedia([OnpcTestUtils.TOP_LAYER, "Deezer"]);
    await tu.openSearchDialog(0, artist);
  ...
  static Future<void> playFromDAB(final OnpcTestUtils tu) async {
    ...
    await tu.findAndTap(() => find.byTooltip("Sets preset wrap-around up"), ensureAfter: () => find.text("ROLAND"));
    expect(find.text("RADIO ROLAND"), findsOneWidget);
    await tu.stepDelayMs();

    // Seek station
    await tu.findAndTap(() => find.byTooltip("Sets tuning frequency wrap-around up"),
    ...
    await tu.findAndTap(() => find.textContaining(station));
  ...
  static Future<void> changeListeningModes(OnpcTestUtils tu) async {
    final String input = "FLAC 44.1 kHz";
    await tu.changeListeningMode(input, Strings.listening_mode_pure_direct, false);
    await tu.changeListeningMode(input, Strings.listening_mode_mode_01, false); // Direct
    await tu.changeListeningMode(input, Strings.listening_mode_mode_00, true); // Stereo
  }
  ...
  static Future<void> changeDeviceSettings(OnpcTestUtils tu) async {
    ...
    await tu.changeFriendlyName(DENON_AVR);
  ...
  static Future<void> allZoneStereo(OnpcTestUtils tu) async {
    ...
    await tu.tester.drag(find.text(lm1), Offset(-200, 0), warnIfMissed: false);
    await tu.stepDelayMs();
    await tu.findAndTap(() => find.text(lm2));
    // Start playing
    await tu.openTab("MEDIA", ensureAfter: () => find.text("NET"));
    await tu.findAndTap(() => find.text("NET"));
    await tu.navigateToMedia([OnpcTestUtils.TOP_LAYER, "Local Music"]);
    await tu.findAndTap(() => find.widgetWithText(ListTile, DENON_AVR), ensureAfter: () => find.byTooltip("Search"));
    await tu.openSearchDialog(1, album);
    ...
    await tu.findAndTap(() => find.text(artist), waitFor: true);
  ...
  static Future<void> createPlayList(OnpcTestUtils tu) async {
    ...
    await tu.findAndTap(() => find.text(artist + " - " + song1), waitFor: true);

    // Create playlist
    await tu.findAndTap(() => find.byTooltip(Strings.playlist_save_queue_as), waitFor: true);
    ...
    await tu.findAndTap(() => find.text(playlist), waitFor: true);
    await tu.findAndTap(() => find.text(artist + " - " + song2), waitFor: true);
    await tu.findAndTap(() => find.text(playlist + " | " + items), waitFor: true);
    await tu.ensureVisible(() => find.text(playlist));
    ...
    await tu.findAndTap(() => find.text("OK"));
    await tu.findAndTap(() => find.text(playlist1), waitFor: true);
    await tu.findAndTap(() => find.text(artist + " - " + song1), waitFor: true);
    await tu.findAndTap(() => find.text(playlist1 + " | " + items), waitFor: true);
