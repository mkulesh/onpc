...
Future<void> _playFromUsb(final OnpcTestUtils tu) async {
  ...
  await tu.findAndTap(() => find.text("Play Queue"), ensureAfter: () => find.textContaining("Play Queue | items: 27"));
...
Future<void> _changeListeningMode(OnpcTestUtils tu, String input, String mode, bool isToneCtrl) async {
  ...
  await tu.findAndTap(() => find.text(mode.toUpperCase()), delay: OnpcTestUtils.NORMAL_DELAY);
  ...
  if (mode == "Stereo") {
    await tu.findAndTap(() => find.byTooltip(Strings.listening_mode_up),
        ensureAfter: () => find.text(Strings.listening_mode_pure_direct));
    await tu.findAndTap(() => find.byTooltip(Strings.listening_mode_down), ensureAfter: () => find.text("Stereo"));
  }
}

Future<void> _openSearchDialog(OnpcTestUtils tu, int type, String search) async {
  await tu.findAndTap(() => find.byTooltip("Search"), ensureAfter: () => find.text("OK"));
